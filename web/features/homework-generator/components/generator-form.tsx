  ...
  return (
    <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_400px]">
      <SurfaceCard className="p-5 sm:p-6" elevated>
        <div className="mb-5 flex items-center justify-between"><div><h2 className="font-semibold tracking-[-0.02em]">Ваш приклад</h2><p className="mt-1 text-[13px] text-copy-muted">Напишіть задачу або вставте кілька завдань.</p></div><span className="rounded-full bg-surface-muted px-2.5 py-1 text-[11px] font-medium text-copy">Математика</span></div>
        <textarea aria-label="Приклад задачі" className="h-[184px] w-full resize-none rounded-lg border border-line-strong bg-surface p-4 text-[15px] leading-6 outline-none transition focus:border-focus focus:ring-2 focus:ring-focus/20" onChange={(event) => onExampleChange(event.target.value)} placeholder="Наприклад: Розв'яжіть рівняння..." value={example} />
